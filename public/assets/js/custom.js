document.addEventListener('DOMContentLoaded', () => {
  ...
  function hasNetWork(online) {
    ...
    if (online) {
      ...
      element.innerHTML =
        ...
        "</div>" +
        "<p class='msg-status'>اتصال اینترنت برقرار نیست</p>" +
        "</div>" +
  ...
  document.addEventListener("click", function (event) {
    if (!event.target.matches(".btn-fullScreen")) return;
    ...
    if (fullscreenElement) {
      ...
      launchIntoFullscreen(document.getElementById("item-fullScreen"));
    }
  });
